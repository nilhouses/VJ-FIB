...
{
	// Alternativa segura: usar una projecció base coneguda
	// O millor: elimina aquest mètode i força sempre passar la proj
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

	// En comptes de glGetIntegerv, agafa la mida de la finestra
	int w, h;
	GLFWwindow* win = glfwGetCurrentContext();
	glfwGetFramebufferSize(win, &w, &h);
	glm::mat4 proj = glm::ortho(0.f, float(w), float(h), 0.f);
	render(c, pixel, size, color, proj);
}

// ── Single char, explicit projection ────────────────────────────────────────
void Text::render(char c, const glm::vec2& pixel, int size, const glm::vec4& color, const glm::mat4& proj)
{
	glm::mat4 modelview;
...
// ── String, explicit projection ──────────────────────────────────────────────
void Text::render(const string& str, const glm::vec2& pixel, int size, const glm::vec4& color, const glm::mat4& proj)
{
	glm::mat4 modelview;
